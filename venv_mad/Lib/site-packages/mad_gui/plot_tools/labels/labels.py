...
class SynchronizationLabel(BaseRegionLabel):
    def __init__(self, start, end, parent=None):
        ...
        # Forzar la posición exacta en segundos en la interfaz gráfica
        if parent is not None:
            # 1. Si el padre es un SensorPlot (tiene plot_data con sampling_rate_hz)
            if (
                hasattr(parent, "plot_data")
                and parent.plot_data is not None
                and getattr(parent.plot_data, "sampling_rate_hz", None)
            ):
                fs = parent.plot_data.sampling_rate_hz
                self.setRegion([start / fs, end / fs])
            # 2. Si el padre es VideoPlot (start y end vienen en ms o segundos)
            else:
                # Si los valores vienen en milisegundos (> 1000), convertir a segundos
                if start > 500 or end > 500:
                    self.setRegion([start / 1000.0, end / 1000.0])
                else:
