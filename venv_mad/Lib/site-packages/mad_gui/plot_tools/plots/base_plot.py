...
class BasePlot(pg.PlotWidget):
    ...
    def set_graph_position(self, percent_since_start: float):
        ...
        if self.state.mode == "sync":
            return
        # jhm: if not the main plot or no sync mode, then no overwrite zoom value
        if not getattr(self, "is_main_plot", False) and not getattr(Config.settings, "SENSORS_SYNCHRONIZED", False):
            return
        sec = self._percent_to_position(percent_since_start)
    ...
    def finish_syncing(self):
        if self.sync_item:
            is_sensor = hasattr(self, "plot_data") and self.plot_data is not None and self.plot_data.data is not None
            fs = getattr(self.plot_data, "sampling_rate_hz", 1.0) if is_sensor else 1000.0
            if not fs or fs <= 0:
                fs = 1.0

            # getRegion devuelve coordenadas en segundos
            start_sec, end_sec = self.sync_item.getRegion()

            if is_sensor:
                val_start = start_sec * fs
                val_end = end_sec * fs
            else:
                val_start = start_sec * 1000.0
                val_end = end_sec * 1000.0

            self.sync_info = pd.Series(
    ...
    def add_sync_item(self):
        ...
        title_text = title.text if title else "Plot"
        print(f"\n[DEBUG add_sync_item] >> Botón Pulsado / Modo Sync en: '{title_text}'")
        ...
        is_video = (self == getattr(self.parent, "video_plot", None)) or (not hasattr(self, "plot_data")) or (self.plot_data is None) or (self.plot_data.data is None)
        print(f"[DEBUG add_sync_item] '{title_text}' -> is_video: {is_video}, self.sync_info: {self.sync_info}")
        ...
        if self.sync_info is not None:
            try:
                raw_start = float(self.sync_info["start"])
                raw_end = float(self.sync_info["end"])
                print(f"[DEBUG add_sync_item] '{title_text}' -> raw_start: {raw_start}, raw_end: {raw_end}")

                if not (pd.isna(raw_start) or pd.isna(raw_end)):
                    if is_video:
                        fps = 29.46
                        if hasattr(self, "video_window") and self.video_window and hasattr(self.video_window, "fps"):
                            fps = self.video_window.fps or 29.46
                        start_sec = raw_start / fps if raw_start > 500 else raw_start
                        end_sec = raw_end / fps if raw_end > 500 else raw_end
                    else:
                        fs = float(getattr(self.plot_data, "sampling_rate_hz", 1.0))
                        print(f"[DEBUG add_sync_item] '{title_text}' -> fs detectada: {fs} Hz")
                        start_sec = raw_start / fs
                        end_sec = raw_end / fs

                    has_valid_sync = True
                    print(f"[DEBUG add_sync_item] '{title_text}' -> RESULTADO CALCULADO: start={start_sec}s, end={end_sec}s")
            except Exception as e:
                print(f"[DEBUG add_sync_item] '{title_text}' -> ERROR procesando sync_info: {e}")
                has_valid_sync = False

        if not has_valid_sync:
            print(f"[DEBUG add_sync_item] '{title_text}' -> SIN DATOS VÁLIDOS: Aplicando 10% y 90% por defecto")
            view_range = self.viewRange()[0]
            x_len = view_range[1] - view_range[0]
            start_sec = view_range[0] + 0.1 * x_len
            end_sec = view_range[0] + 0.9 * x_len

        self.sync_item = SynchronizationLabel(start=0, end=0, parent=self)
        self.addItem(self.sync_item)

        self.sync_item.blockSignals(True)
        self.sync_item.setRegion([start_sec, end_sec])
        self.sync_item.blockSignals(False)
        print(f"[DEBUG add_sync_item] '{title_text}' -> setRegion aplicado: ({start_sec}, {end_sec})")
